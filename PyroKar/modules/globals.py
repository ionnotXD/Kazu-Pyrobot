...
from PyroKar import *
from PyroKar.helpers.adminHelpers import DEVS, WHITELIST
from PyroKar.helpers.basic import edit_or_reply
from PyroKar.helpers.PyroHelpers import get_ub_chats
...
async def gban_user(client: Client, message: Message):
    ...
    if user_id in WHITELIST:
        return await Man.edit(
            "**Kau Gak Bisa Kontol Gban Dia Karena Dia Adalah admin @Html12text 😡**"
        )
    if user_id:
        try:
...
async def gmute_user(client: Client, message: Message):
    ...
    if user.id in WHITELIST:
        return await Man.edit(
            "**Kau Gak Bisa Kontol Gmute Dia Karena Dia Adalah admin @Html12text 😡**"
        )
    try:
